define(require => {
    ...
    function init() {
        ...
        holdResultDelay = gameConfig.collectBonusPickerHoldResultDelay || 2.5;
        holdeDelayBeforeDisable = gameConfig.collectBonusHoldeDelayBeforeDisable || 1;
        
        pickerArray = [
            new PrizeBonusPicker({prefix: 'LEFT', container: displayList.picker_1,index:1}),
            new PrizeBonusPicker({prefix: 'MIDDLE', container: displayList.picker_2,index:2}),
            new PrizeBonusPicker({prefix: 'RIGHT', container: displayList.picker_3,index:3}),
            new PrizeBonusPicker({prefix: 'LEFT', container: displayList.picker_4,index:4}),
            new PrizeBonusPicker({prefix: 'MIDDLE', container: displayList.picker_5,index:5}),
            new PrizeBonusPicker({prefix: 'RIGHT', container: displayList.picker_6,index:6}),
            new PrizeBonusPicker({prefix: 'LEFT', container: displayList.picker_7,index:7}),
            new PrizeBonusPicker({prefix: 'MIDDLE', container: displayList.picker_8,index:8}),
            new PrizeBonusPicker({prefix: 'RIGHT', container: displayList.picker_9,index:9}),
        ];
        ...
        let logoText = new PIXI.Sprite(new PIXI.Texture.fromFrame('collect-bonus-text'));
        logoText.anchor.set(0.5);
        displayList.collectInfoLogoText.addChild(logoText);
    ...
    function enable(data) {
        if (data === "CollectBonus") {
            ...
            displayList.collectInfoLogoText.alpha = 0;
            ...
            Tween.delayedCall(introDelay,async ()=>{
                ...
                Tween.to(displayList.CollectBonusContainer,infoDelay,{
                    alpha:1,
                });
                ...
                await new Promise(resolve => {
                  ...
                  let logoTextStartPosition = displayList.collectInfoLogoText.position;
                  ...
                  tl.fromTo(displayList.collectInfoLogoText, fadeTime, { 
                      alpha : 0,
                      y : logoTextStartPosition.y - shiftAmount
                  },{ 
                      alpha : 1,
                      y : logoTextStartPosition.y,
                      ease : window.Power2.easeOut
                  }, 0);
                ...
                await Promise.all(
                    pickerArray.map(async e=>{
                        ...
                        await e.showValue(scenario.shift(), colourPostFix);
                        e.addToTotal();
                    })
                );
            });
    ...
    function end() {
        ...
        Tween.delayedCall(holdResultDelay,()=>{
            ...
            if(nextStage === 'BaseGame'){
                ...
                msgBus.publish('Game.TransitionToBonus', { fromStage : 'CollectBonus', toStage : 'WheelBonus', callback : onGameCompleteCallback });
            }
    ...
    function addToTotalWinValue(data) {
        totalWinValue += data;
        displayList.collectInfoTotalWinValue.text = SKBeInstant.formatCurrency(totalWinValue).formattedAmount;
    }
