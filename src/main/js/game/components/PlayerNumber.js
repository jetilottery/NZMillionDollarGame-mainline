define(require => {
  ...
  class PlayerNumber extends NumberCard {
    constructor() {
      ...
      this.valueText.y = SKBeInstant.config.assetPack === 'mobile' ? TEXT_Y_POS_MOBILE : TEXT_Y_POS;
      this.valueText.maxWidth = MAX_VALUE_WIDTH;
      this.winText.addChild(this.valueText);
    ...
    presentWin() {
      if(this.revealState === 'Gold'){
        ...
        Tween.delayedCall(0.35, function showRevealedGoldNumberFromSilver(){
          ...
          _globalScope.valueText.style = textStyles.parse('prizeValueWin'+_globalScope.fontSizeFix);
    ...
    presentInstantWin() {
        this.valueText.style = textStyles.parse('prizeValueWin'+this.fontSizeFix);
      ...
      if(BONUS_SYMBOLS.includes(this.revealState)){
        if(this.revealState ==='WheelBonus'){
          ...
          if(this.revealState === 'V') {
              ...
              this.resultAnimBackground.state.setAnimation(0,'NumBGREVEAL', false);
              this.resultAnimBackground.state.addAnimation(0,'NumBGSTATIC', false, 0);
              Tween.to(this.valueText, 0.2, { alpha : 1});
          }
      }
    }
    ...
    populate([numberOrSymbol, value], goldOrSilver) {
      ...
      if(isNaN(numberOrSymbol)){
        ...
        this.valueText.style = goldOrSilver === 'Silver' ? textStyles.parse('prizeValueNoWin'+this.fontSizeFix) : textStyles.parse('prizeValueWin'+this.fontSizeFix);
        this.valueText.text = SKBeInstant.formatCurrency(value).formattedAmount;      
        this.resultAnimBackground = new PIXI.spine.Spine(resLib.spine['NumberBG'].spineData);
        this.resultContainer.addChildAt(this.resultAnimBackground, 0);
