define(require => {
    ...
    const bonusSpriteTable = {
        'WheelBonus': {
            symbol : 'wheel-bonus-symbol',
            text : 'wheel-bonus-text'
        },

        'CollectBonus': {
            symbol : 'collect-bonus-symbol',
    ...
    function playShowerAnimation(){
      _transitionLand.state.setAnimation(0, 'GoldenShower', false);
      _transitionPort.state.setAnimation(0, 'PORTRAIT_GoldenShower', false);
        ...
        if (_toBonus) {
          ...
          let gameFadeTime = 0.8;
            ...
            if(fromStage === 'BaseGame'){
                Tween.to(displayList.ticketSelectBarSmall,gameFadeTime,{alpha:0,onComplete:()=>{displayList.ticketSelectBarSmall.interactive = false;}});
    ...
    function swapScreen() {
        if (_toBonus) {
            ...
            if(SKBeInstant.config.gameConfigurationDetails.availablePrices.length > 1) {
                Tween.to(displayList.ticketSelectBarSmall,0.3,{alpha:1,onComplete:()=>{displayList.ticketSelectBarSmall.interactive = true;}});
            }
        }
